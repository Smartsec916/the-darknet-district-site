...
(async()=>{
 ...
 for(const signedIn of [false,true]){
  ...
  if(signedIn){await page.evaluate(()=>{leaveMenu();mode='dock';VoidAccount.request('account');});await page.getByRole('button',{name:'SIGN IN WITH GOOGLE',exact:true}).click();await page.waitForFunction(()=>VoidAccount.user&&!VoidAccount.busy&&ownedGear.length===2);await page.evaluate(()=>title());}
  await page.getByRole('button',{name:'START NEW CAMPAIGN',exact:true}).click();await page.getByRole('button',{name:'START NEW CAMPAIGN',exact:true}).click();
 ...
 await page.evaluate(()=>{state.completed=1;dock();});await page.evaluate(()=>{leaveMenu();mode='dock';VoidAccount.request('account');});await page.getByRole('button',{name:'SIGN IN WITH GOOGLE',exact:true}).click();await page.waitForFunction(()=>VoidAccount.user&&!VoidAccount.busy);
 ...
 await page.evaluate(()=>{state.completed=1;dock();});await page.evaluate(()=>{leaveMenu();mode='dock';VoidAccount.request('account');});await page.getByRole('button',{name:'SIGN OUT',exact:true}).click();await page.waitForFunction(()=>!VoidAccount.busy);admin=true;
 ...
 await page.evaluate(async()=>{await keplerStationTexture.load();await keplerStationTexture.decode();trialGear=null;devMissileTrial=false;state.location='kepler';state.quest='return';dock();});if(out)await page.screenshot({path:path.join(out,'kepler-interior.png')});

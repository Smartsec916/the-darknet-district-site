...
(async()=>{
 ...
 await page.evaluate(()=>{for(let i=0;i<6;i++)advanceSpeech();});await page.locator('[data-action="story-choice:0"]').click();await page.evaluate(()=>{advanceSpeech();advanceSpeech();});await page.waitForFunction(()=>mode==='play');assert.equal(await page.evaluate(()=>mode),'play');assert.equal(await page.evaluate(()=>state.story.flags.maraPromise),true);
 await page.keyboard.press('Escape');assert.equal(await page.evaluate(()=>mode),'menu');await page.locator('[data-action="menu-settings"]').click();await page.locator('[data-action="menu-back"]').click();await page.locator('[data-action="menu-resume"]').click();await page.waitForFunction(()=>mode==='play');assert.equal(await page.evaluate(()=>mode),'play');
 await page.keyboard.press('Escape');await page.locator('[data-action="menu-new"]').click();await page.getByText(/Existing campaign data/).waitFor();await page.locator('[data-action="menu-back"]').click();await page.locator('[data-action="menu-load"]').click();await page.locator('[data-action="menu-load-local"]').click();await page.waitForFunction(()=>mode==='play');assert.equal(await page.evaluate(()=>mode),'play');

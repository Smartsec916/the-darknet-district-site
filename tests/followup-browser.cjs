...
(async()=>{
 ...
 await page.evaluate(()=>{state.quest='open';state.completed=2;shop();});assert.equal(await page.locator('.shop-placeholder').count(),1);assert.equal(await page.locator('[data-action^="purchase:"]').count(),0);assert.equal(await page.locator('[data-action^="buy:"]').count(),4);assert.equal(await page.locator('[data-action^="credit-gear:"]').count(),3);if(out)await page.screenshot({path:path.join(out,'placeholder-shop.png')});

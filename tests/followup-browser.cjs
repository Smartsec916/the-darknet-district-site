...
(async()=>{
 ...
 await page.goto('http://127.0.0.1:5000/void-runner.html?renderer=legacy');await page.waitForFunction(()=>window.VoidStartup?.started);await page.waitForTimeout(120);
 ...
 await page.evaluate(()=>{state=C.fresh();state.quest='arrival';launch();mode='pause';elapsed=current.duration;approachTime=3;flight.route.phase="arrived";artDraws=[];draw();});assert(await page.evaluate(()=>artDraws.includes('station.png')));
 ...
 await page.evaluate(()=>{state.quest='open';state.completed=2;shop();});assert.equal(await page.locator('.shop-placeholder').count(),1);assert.equal(await page.locator('[data-action^="purchase:"]').count(),1);assert.equal(await page.locator('[data-action^="buy:"]').count(),4);assert.equal(await page.locator('[data-action^="credit-gear:"]').count(),3);if(out)await page.screenshot({path:path.join(out,'placeholder-shop.png')});
 await page.setViewportSize({width:390,height:844});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 const missing=await browser.newPage(),warnings=[];missing.on('console',m=>{if(m.type()==='warning')warnings.push(m.text())});await missing.route('**/art/station.png',r=>r.fulfill({status:404,body:'missing'}));await missing.goto('http://127.0.0.1:5000/void-runner.html?renderer=legacy');await missing.waitForFunction(()=>window.VoidStartup?.started);await missing.evaluate(()=>{state.quest='arrival';launch();});await missing.waitForFunction(()=>stationTexture.complete);assert(warnings.some(w=>w.includes('/void-runner/art/station.png')));assert.equal(await missing.evaluate(()=>VoidStartup.started),true);
 ...
 await staticPage.goto('http://127.0.0.1:5001/void-runner.html?renderer=legacy');await staticPage.waitForFunction(()=>window.VoidStartup?.started);
 ...
 const noCockpit=await browser.newPage();await noCockpit.route('**/art/cockpit-kestrel.png',r=>r.fulfill({status:404,body:'missing'}));await noCockpit.goto('http://127.0.0.1:5000/void-runner.html?renderer=legacy');await noCockpit.getByText('Game artwork unavailable').waitFor();assert.equal(await noCockpit.evaluate(()=>VoidStartup.started),false);

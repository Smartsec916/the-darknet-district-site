...
(async()=>{
 ...
 const missing=await browser.newPage(),warnings=[];missing.on('console',m=>{if(m.type()==='warning')warnings.push(m.text())});await missing.route('**/art/station.png',r=>r.fulfill({status:404,body:'missing'}));await missing.goto('http://127.0.0.1:5000/void-runner.html');await missing.waitForFunction(()=>window.VoidStartup?.started);await missing.evaluate(()=>{state.quest='arrival';launch();});await missing.waitForFunction(()=>stationTexture.complete);assert(warnings.some(w=>w.includes('/void-runner/art/station.png')));assert.equal(await missing.evaluate(()=>VoidStartup.started),true);

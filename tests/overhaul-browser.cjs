...
(async()=>{
 ...
 await page.goto('http://127.0.0.1:5000/void-runner.html?renderer=legacy');await page.waitForFunction(()=>VoidStartup.started);
 ...
 await page.locator('[data-action=menu-new]').click();await page.locator('[data-action=story-intro]').click();await page.getByRole('button',{name:'CONTINUE →',exact:true}).click();await page.getByRole('button',{name:'CONTINUE →',exact:true}).click();await page.getByRole('button',{name:'RESPOND →',exact:true}).click();await page.locator('[data-action="story-choice:0"]').click();await page.getByRole('button',{name:'BOARD SHIP →',exact:true}).click();
 ...
 async function completeRoute(){return page.evaluate(()=>{
   ...
   for(let i=0;i<5000&&['play','pause'].includes(mode);i++){
    mode='play';VoidInput.held.clear();flight.mouseX=flight.mouseY=0;
 ...
 await page.getByRole('button',{name:'EXIT TO SPACE STATION →',exact:true}).click();await page.getByRole('button',{name:'GO TO BAR →',exact:true}).click();await page.getByRole('button',{name:'ROOK · TALK',exact:true}).click();await page.getByRole('button',{name:'ACCEPT JOB →',exact:true}).click();await page.getByRole('button',{name:'LAUNCH →',exact:true}).click();
 ...
 await page.getByRole('button',{name:'GO BACK TO YOUR SHIP →',exact:true}).click();await page.getByRole('button',{name:'RETURN TO MERIDIAN →',exact:true}).click();await completeRoute();await page.getByRole('button',{name:'EXIT TO SPACE STATION →',exact:true}).click();await page.getByRole('button',{name:'GO TO BAR →',exact:true}).click();await page.getByRole('button',{name:'ROOK · TALK',exact:true}).click();await page.getByRole('button',{name:'ACCEPT JOB →',exact:true}).click();await page.getByRole('button',{name:'LAUNCH →',exact:true}).click();
 const second=await completeRoute();assert.equal(second.combatRuns,2);assert.equal(second.travel,null);assert.equal(await page.evaluate(()=>speech.lines.filter(l=>l.who==='rook').length),1);await page.getByRole('button',{name:'NEXT →',exact:true}).click();assert((await page.locator('#spoken-text').innerText()).includes('missile launcher'));await capture('rook-missile-offer');await page.getByRole('button',{name:'GO BACK TO YOUR SHIP →',exact:true}).click();await page.evaluate(()=>shop());
 ...
 await page.evaluate(()=>hangar());await capture('hangar-starter');
 ...
 for(const id of ['ship2','ship3','starter']){
  ...
  const flightTest=await page.evaluate(()=>{C.chooseDestination(state,state.location==='kepler'?'meridian':'kepler');launch();mode='play';flight.route.phase='align';VoidInput.held.add('KeyD');for(let i=0;i<15;i++)update(.04);VoidInput.held.clear();mode='pause';draw();return {ship:state.activeShip,rate:flight.yawRate,cockpit:VoidShips.get(state).cockpit,audio:VoidShips.get(state).audio,capacity:missileState.missileCapacity};});profiles.push(flightTest);await capture(id+'-cockpit');await page.evaluate(()=>{mode='dock';hangar();});
 }
 ...
 await page.evaluate(()=>{mode='dock';C.switchShip(state,'ship3');save();openGameMenu();settingsPage();});
 await page.locator('[data-setting=voice]').fill('50');await page.reload();await page.waitForFunction(()=>VoidStartup.started);
 ...
 await page.setViewportSize({width:390,height:844});await page.evaluate(()=>hangar());assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await capture('hangar-mobile');

...
(async()=>{
 ...
 await page.getByRole('button',{name:'BUY SHIP / 2500 CR',exact:true}).click();assert.equal(await page.evaluate(()=>state.credits),0);await page.getByRole('button',{name:'OPEN HANGAR',exact:true}).click();
 const profiles=[];
 for(const id of ['ship2','ship3','starter']){
  ...
  const flightTest=await page.evaluate(()=>{C.chooseDestination(state,state.location==='kepler'?'meridian':'kepler');launch();mode='play';keys.add('KeyD');for(let i=0;i<15;i++)update(.04);keys.clear();mode='pause';draw();return {ship:state.activeShip,rate:flight.yawRate,cockpit:VoidShips.get(state).cockpit,audio:VoidShips.get(state).audio,capacity:missileState.missileCapacity};});profiles.push(flightTest);await capture(id+'-cockpit');await page.evaluate(()=>{mode='dock';hangar();});
 }
 ...
 assert.equal(await page.evaluate(()=>state.activeShip),'ship3');assert.equal(await page.evaluate(()=>state.ownedShips.length),3);assert.equal(await page.evaluate(()=>VoidAudio.settings.voice),.5);assert.equal(await page.evaluate(()=>state.missileOfferSeen),true);

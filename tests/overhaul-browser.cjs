...
(async()=>{
 ...
 const second=await completeRoute();assert.equal(second.combatRuns,2);assert.equal(second.travel,null);assert.equal(await page.evaluate(()=>speech.lines.filter(l=>l.who==='rook').length),1);await page.getByRole('button',{name:'NEXT →',exact:true}).click();assert((await page.locator('#spoken-text').innerText()).includes('missile launcher'));await capture('rook-missile-offer');await page.getByRole('button',{name:'OPEN STATION MENU →',exact:true}).click();await page.evaluate(()=>shop());

...
Keep the original renderer available. Babylon is the default for new players and can be explicitly selected with `?renderer=babylon`; `?renderer=legacy` is the recovery path. The engine is locally hosted, version pinned and lazy loaded. No paid services or new backend infrastructure.
...
Ship `relationship` is `friendly`, `neutral` or `hostile`; `VoidStory.relationship` accepts older scripted `allegiance` values for compatibility. An absent value retains legacy hostile behavior. Existing lock, laser, drone, missile and route-clear rules remain authoritative. `traffic.js` owns a separate ambient contact collection; contacts never enter the enemy target list. Half of eligible peaceful departures are empty; the rest have at most three neutral contacts with independent trajectories, different start times/distances/speeds, and bounded lifetimes. Hostile routes and pending hostile story encounters suppress civilians. Random friendly ships are not generated. HUD brackets use green for friendly, gray for neutral and red for hostile. The original renderer remains capable of showing story-defined allied contacts.
...
Before a Babylon departure, initialize the engine, prepare the current exterior, wait for sky/model loading and scene readiness, and decode the canonical departure artwork. Only then call the existing launch chain. The planet and rocks exist during the departure phase; the origin station is behind the player and eligible traffic is independent; transparent hangar artwork stays over them as the ship leaves. There is no phase that deliberately removes the exterior. Destination preparation occurs behind a warp synchronization overlay while simulation is held. Required asset failure keeps the pilot at a recoverable preparation screen, with retry/original-renderer controls. Preparation uses `preparation.js`: module download 90 seconds, asset batches 60 seconds, scene readiness 30 seconds, departure artwork 45 seconds, and an overall 180-second ceiling. Aborted tasks cannot commit late initialization or model results. Fallback never changes saved preference until the player chooses it. Failures log phase, asset, error, readiness and elapsed time. Menus cannot capture a half-prepared transition.
...
## Ambient traffic and station ownership (current update)

`VoidTraffic.definitions` specifies model, neutral relationship, role, crossing behavior, speed/lifetime ranges, spawn distance/altitude and marker policy. `VoidTraffic.config` controls empty-area probability and maximum contacts. Only the two lightweight current roles are provided; no new mission types or random allies are introduced. `step` advances inertial velocity and subtracts player displacement rather than steering toward the camera. Near the end of a visit, contacts accelerate, stretch, show a Babylon warp wake and fade before removal. Shared materials are reused; per-contact transforms and wake meshes are disposed when contacts leave. Ambient markers have no objective arrows, no health bars, and stay out of the central aiming box.

Origin station position integrates player displacement from a point behind the departure door. Destination station visibility requires the destination scene and the existing late-warp reveal threshold; it is hidden during interdiction. Traffic is cleared at warp boundaries before combat, and both traffic and Babylon environment nodes are released on docking. Rendering and movement remain in the existing single animation loop. Legacy sky WebGL initialization is deferred until legacy flight is actually drawn.

The obsolete first-delivery login promotion and `loginOfferSeen` serialization are removed. Old saves containing that field remain accepted and normalize without it. Accounts, guest play, cloud saves and the upgrade shop retain their existing entry points.

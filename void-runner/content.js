...
(function(root) {
  ...
  const missions = definitions.map((d,i) => ({id:d[0],name:d[1],chapter:d[2],kind:d[3],destination:d[4],enemies:d[5],tier:d[6],duration:d[7],reward:d[8],briefing:d[9],cargo:d[3]==='escort'?'Rescue shuttle':d[3]==='salvage'?'Recovered intelligence':'Sector clearance',legal:true,requires:d.length>10?d[10]:i?definitions[i-1][0]:null,contact:'Rook'}));
  ...
  const gear = {
    ...
    ghost: {name:'Ghost Drive',slot:'utility',description:'A 0.7-second invulnerable speed burst. Use the Drive binding or tap DRIVE. 8-second cooldown.',symbol:'G',color:'#ffbd69'},
  ...
  const creditGear = {
    launcher: {name:'Missile launcher',slot:'missile',price:0,description:'Rook’s installation voucher. Six missiles in the Kestrel; larger racks on upgraded ships. Refilled at departure. Hold a hostile in the targeting ring until LOCKED, then use the Missile binding (default F) or right-click.',color:'#ffbd69'},
    vector: {name:'Vector Booster',slot:'utility',price:500,description:'A 0.4-second evasive burst. 12-second cooldown. Use the Drive binding or tap DRIVE.',art:'ghost',color:'#ffbd69'},

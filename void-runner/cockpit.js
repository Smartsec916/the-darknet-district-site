...
launch=function(){cockpitLaunch();if(mode!=='play')return;Object.assign(flight,{yaw:0,pitch:0,roll:0,yawRate:0,pitchRate:0,throttle:.8,mouseX:0,mouseY:0,travel:0,nav:{x:0,y:0,z:140}});$('route-status').textContent=`0% / ${current.enemies} HOSTILES`;$('weapon').textContent=C.stats(state).piercing?'WRAITH / PIERCING':`PULSE MK ${state.upgrades.guns+1}`;flight.rocks=Array.from({length:34},(_,i)=>({x:(i%2?1:-1)*(28+random()*100),y:(random()-.5)*120,z:25+random()*300,size:1.5+random()*6,phase:random()*6}));announce('Cockpit online · WASD / arrows turn · Shift / X throttle · Space fires');};
...
spawnEnemy=function(){cockpitSpawn();const e=enemies[enemies.length-1],b=flightBasis();const spread=e.x*4;e.x=b.f.x*115+b.r.x*spread+b.u.x*e.y*3;e.y=b.f.y*115+b.r.y*spread+b.u.y*e.y*3;e.z=b.f.z*115+b.r.z*spread;const d=FM.unit({x:-e.x,y:-e.y,z:-e.z});e.velocity={x:d.x*19,y:d.y*19,z:d.z*19};e.passTime=0;};
...
function hitEnemy(e,damage){if(e.dead)return;if(!e.generator&&enemies.some(q=>q.generator&&!q.dead))return;e.armor-=damage;if(e.armor<=0){e.dead=true;resolved++;burst(e);}}
function cockpitMission(dt,velocity){
 ...
 for(const o of missionObjects){moveRelative(o,velocity,dt);const distance=FM.length(o);if(distance<(o.kind==='salvage'?7:4)){if(o.kind==='salvage'){objectiveCount++;announce(`Signal recovered / ${Math.min(3,objectiveCount)} of 3`);}else hurt(18);o.dead=true;}if(distance>230)o.dead=true;}
 missionObjects=missionObjects.filter(o=>!o.dead);
 if(current.kind==='escort'){escortClock-=dt;if(escortClock<=0&&enemies.length){const e=enemies[0];hostile.push({...e,vx:0,vy:0,vz:0,life:5,escort:true,damage:10});const h=hostile[hostile.length-1],d=FM.unit({x:-e.x,y:5-e.y,z:25-e.z});Object.assign(h,{vx:d.x*40,vy:d.y*40,vz:d.z*40,dead:false});escortClock=3.5;}}
 updateEquipmentHud();
}
const cockpitIdleUpdate=update;
update=function(dt){
 if(mode!=='play'){cockpitIdleUpdate(dt);return;}
 time+=dt;noticeTime-=dt;if(noticeTime<=0)$('notice').textContent='';damageTime=Math.max(0,damageTime-dt);
 ...
 shot-=dt;if((keys.has('Space')||firing||touchFiring)&&shot<=0){shot=stats.cooldown;for(const offset of [-.65,.65])bullets.push({x:b.r.x*offset,y:b.r.y*offset,z:b.r.z*offset,vx:b.f.x*180,vy:b.f.y*180,vz:b.f.z*180,damage:stats.damage,life:2.5});tone(600,.055);}
 ...
 for(const e of enemies){
  ...
  if(e.fire<=0&&distance<165){e.fire=e.boss?1.4:e.heavy?2:Math.max(1,3-current.tier*.2);const d=FM.unit({x:-e.x,y:-e.y,z:-e.z});for(const offset of e.boss?[-.07,0,.07]:[0])hostile.push({x:e.x,y:e.y,z:e.z,vx:(d.x+offset)*55,vy:d.y*55,vz:d.z*55,damage:e.heavy?20:10,life:5});}
  if(distance<3+e.size&&!e.colliding){hurt(e.heavy?25:12);e.colliding=true;}if(distance>12)e.colliding=false;
 }
 for(const shot of bullets){const from={x:shot.x,y:shot.y,z:shot.previousZ??shot.z};shot.life=(shot.life??2)-dt;if(shot.target&&!shot.target.dead){const d=FM.unit({x:shot.target.x-shot.x,y:shot.target.y-shot.y,z:shot.target.z-shot.z});shot.vx=d.x*180;shot.vy=d.y*180;shot.vz=d.z*180;}shot.x+=(shot.vx||0)*dt;shot.y+=(shot.vy||0)*dt;shot.z+=(shot.vz??180)*dt;moveRelative(shot,velocity,dt);
 ...
 for(const h of hostile){const from={x:h.x,y:h.y,z:h.z};h.x+=(h.vx||0)*dt;h.y+=(h.vy||0)*dt;h.z+=(h.vz??-55)*dt;moveRelative(h,velocity,dt);h.life=(h.life??5)-dt;if(h.escort){if(FM.segmentHit(from,h,{x:0,y:5,z:25},4)){h.x=0;h.y=2.4;escortImpact(h);h.dead=true;}}else if(FM.segmentHit(from,h,{x:0,y:0,z:0},1.8)){hurt(h.damage);h.dead=true;}}
...
function cockpitEnemy(e){const p=flightPoint(e);if(p.z<1)return;const size=Math.min(H*.55,p.s*e.size*4.2),frame=fleetFrames[e.className]||fleetFrames.raider;if(p.x< -size||p.x>W+size||p.y< -size||p.y>H+size)return;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(-flight.roll+Math.sin(e.age*.7+e.phase)*.2);if(fleetTexture.complete&&fleetTexture.naturalWidth)ctx.drawImage(fleetTexture,...frame,-size/2,-size/2,size,size);else{ctx.fillStyle='#db7994';ctx.beginPath();ctx.moveTo(0,size*.5);ctx.lineTo(-size*.4,-size*.4);ctx.lineTo(size*.4,-size*.4);ctx.fill();}ctx.restore();ctx.strokeStyle=e.generator?'#58ffe1':'#ff728a';const r=Math.max(12,size*.55);ctx.strokeRect(p.x-r,p.y-r,r*2,r*2);ctx.fillStyle=ctx.strokeStyle;ctx.fillRect(p.x-r,p.y-r-6,r*2*Math.max(0,e.armor/e.maxArmor),2);if(e.generator||e.boss){ctx.font='12px Consolas';ctx.fillText(e.generator?'SHIELD RELAY':'CAPITAL SHIP',p.x-r,p.y-r-14);}}
...
draw=function(){
 if(mode!=='play'&&mode!=='pause'){cockpitDockDraw();return;}

...
function hud() { $('credits').textContent = state.credits.toLocaleString(); const max = C.stats(state).hull; $('hull').style.width = Math.max(0, hp / max * 100) + '%'; $('hull-number').textContent = `${Math.ceil(hp)} / ${max}`; $('weapon').textContent = `PULSE MK ${state.upgrades.guns + 1} / THRUST MK ${state.upgrades.engines + 1}`; }

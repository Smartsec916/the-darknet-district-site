...
drawWarpMarker=function(){
 ...
 for(const r of flightRoutes()){const p=flightPoint({x:r.vector.x*140,y:0,z:r.vector.z*140});if(p.z<=0)continue;const interstellar=VoidUniverse.route(state.location,r.id).kind==='interstellar',tracked=VoidMissions.active(state,C).find(m=>m.id===state.universe.trackedMission),guide=VoidMissions.guidance(state,tracked,false),isTracked=guide?.destination===r.id;ctx.save();ctx.translate(p.x,p.y);ctx.strokeStyle=isTracked?'#f4d15d':interstellar?'#bda0e0':'#83c4b5';ctx.fillStyle=ctx.strokeStyle;ctx.lineWidth=current.destination===r.id?3:1;ctx.beginPath();for(let i=0;i<=6;i++){const a=i*Math.PI/3;const radius=interstellar?27:18;i?ctx.lineTo(Math.cos(a)*radius,Math.sin(a)*radius):ctx.moveTo(Math.cos(a)*radius,Math.sin(a)*radius);}ctx.stroke();ctx.font='11px monospace';ctx.textAlign='center';ctx.fillText((isTracked?'◆ TRACKED / ':'')+(interstellar?'INTERSTELLAR / ':'')+C.stations[r.id].name.toUpperCase(),0,43);ctx.restore();}
};

...
(function(root){
 const config={alignmentCos:Math.cos(8*Math.PI/180),hold:.65,warpSeconds:6,interruption:.42};
 function create(origin,destination,mission,encounter=false){const heading={meridian:-.55,kepler:.65,undertow:-.8,foundry:.9}[destination]||.55;return {origin,destination,mission:mission||null,progress:0,phase:'align',aligned:0,encounter:{type:encounter?'combat':'none',state:encounter?'pending':'cleared'},vector:{x:Math.sin(heading),y:0,z:Math.cos(heading)}};}

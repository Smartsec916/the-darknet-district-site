...
function drawStationStructure(id,x,y,width){
 ctx.save();ctx.translate(x,y);ctx.scale(width/800,width/800);ctx.rotate(id==='kepler'?-.16:id==='undertow'?.12:0);
 const palette={kepler:['#d5dfdf','#62767c','#ffdd91'],undertow:['#4e424c','#211c2d','#ec77de'],foundry:['#635a51','#241d1b','#ff9a43']}[id];
 ...
 if(id==='kepler'){
   for(const ry of [-35,40]){ctx.lineWidth=24;ctx.strokeStyle=metal;ctx.beginPath();ctx.ellipse(0,ry,285,95,0,0,Math.PI*2);ctx.stroke();ctx.lineWidth=2;ctx.strokeStyle=palette[2];ctx.stroke();}
   for(let i=0;i<8;i++){const a=i*Math.PI/4;ctx.strokeStyle='#91a2a8';ctx.lineWidth=8;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(Math.cos(a)*280,Math.sin(a)*95);ctx.stroke();}
   box(-28,-150,56,270);box(-11,-204,22,70);box(-14,122,28,74);
   for(const side of [-1,1])box(side*250-35,10,70,38);
 }else if(id==='undertow'){
   box(-260,-12,510,27);box(-32,-110,58,225);
   for(let i=0;i<9;i++){const bx=-260+i*62,by=(i%3-1)*60;ctx.strokeStyle='#685569';ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(bx+20,0);ctx.lineTo(bx+20,by);ctx.stroke();box(bx,by,40+(i%2)*18,25+(i%3)*12);ctx.fillStyle='#967149';ctx.fillRect(bx+5,by+3,12,9);}
   box(-125,-148,48,60);box(90,74,76,38);ctx.strokeStyle=palette[2];ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-230,-75);ctx.lineTo(240,100);ctx.stroke();
 }else{
